...
class ModelServiceUltralytics implements ModelService {
  ...
  Future<void> loadModel() async {
    ...
    try {
      ...
      _yolo = YOLO(
        modelPath: 'best_float32', // Model name without .tflite extension
      ...
      addLog('   Model path: best_float32');
  ...
  Future<DetectionResult> detectAndClassify(
    ...
    try {
      ...
      // According to API docs, predict() returns Map<String, dynamic>
      // with 'boxes' key containing List of detections
      ...
      // Extract boxes from results - API might return 'boxes' or 'detections' key
      // Try 'detections' first (seems to be the actual key based on API), then fallback to 'boxes'
      ...
      if (results.containsKey('detections')) {
        ...
        // Try to find any list in the results
      ...
      for (int i = 0; i < boxesList.length; i++) {
        ...
        try {
          ...
          // Extract detection data from box map
          // YOLO API might return different formats - handle multiple possibilities
          final boxMap = boxData as Map<String, dynamic>;

          // Log all keys and their values to understand the format
          ...
          // Try different possible key names for class
          ...
          // Based on logs, the structure has 'boundingBox' and 'normalizedBox' keys
          double x1 = 0.0, y1 = 0.0, x2 = 0.0, y2 = 0.0;

          // Try to get boundingBox first (pixel coordinates)
          if (boxMap.containsKey('boundingBox')) {
            final bbox = boxMap['boundingBox'];
            addLog('     Found boundingBox: $bbox (${bbox.runtimeType})');

            if (bbox is Map) {
              // It's a Map with left, top, right, bottom or x, y, width, height
              ...
              // It's a list [x1, y1, x2, y2] or [left, top, right, bottom]
          ...
          // If boundingBox didn't work, try normalizedBox (normalized 0-1 coordinates)
          if ((x1 == 0.0 && y1 == 0.0 && x2 == 0.0 && y2 == 0.0) &&
            ...
            if (nbox is Map) {
              ...
              if (nboxMap.containsKey('left') &&
                ...
                // Convert normalized center to pixel corners
          ...
          // Fallback: try direct keys in boxMap
          if (x1 == 0.0 && y1 == 0.0 && x2 == 0.0 && y2 == 0.0) {
            addLog(
          ...
          // Use class name provided by detection box directly.
          // Some models/plugins always return classIndex 0, but className is correct.
          // Only fall back to labels.txt if detection did not supply a name.
